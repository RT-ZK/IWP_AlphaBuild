...
namespace Platformer.Mechanics
{
    ...
    public class PlayerController : KinematicObject
    {
        ...
        protected override void FixedUpdate()
        {
            ...
            velocity.y += externalForces.y * Time.deltaTime;
            
            velocity.x = targetVelocity.x + externalForces.x * Time.deltaTime;
        ...
        public void RocketJump(Vector3 projposition)
        {
            ...
            if (dist < 5 && launchTimer == 0.0f)
            {
                launchTimer = 1.5f;
                float y = projposition.y - transform.position.y;
                Debug.Log(y);
                float x = projposition.x - transform.position.x;
                Debug.Log(x);
